...
void Bullet::update(float& time, Map& map)
{
    ...
    if (life)
    {
        ...
        for (int i = y / 32; i < (y + h) / 32; i++)//проходимся по элементам карты
            ...
            {
                if (TileMap[i*l_map+j] == '0') //если элемент тайлик стены, то
                    life = false;         //то пуля исчезает
                if (TileMap[i*l_map+j] == 'x') { //если тайлик кирпичной стены и пуля игрока
                    if (name == "HeroBullet") TileMap[i*l_map+j] = 'X';  // то этот кусок стены разрушается
